...
class GenerateHouseMapView(APIView):
    ...
    def generate_villa(self, data):
        return f'''Generate a single story modern villa sketch floor plan measuring { data.get('total_land_dimension')}.
    ...
    living room ({data.get('living_room_dimension') } feet), an open kitchen ({ data.get('kitchen_dimension')}), a dining area ({data.get('dining_room_dimension')} feet), and a large balcony ({data.get('balcony_dimension')} feet).
     Additionally, a landscaped garden, a two-car garage ({data.get('garage_dimension') }),
     and a front porch. The layout should be functional, aesthetically appealing, and clearly labeled, following a top-down
     architectural blueprint style with black-and-white technical detailing.
     Keep vastu shashtra in mind.
     Make sure dimensions are clearly mentioned as if your job depends on the clarity of dimensions.
        '''
